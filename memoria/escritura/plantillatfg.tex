...
Basándose en los resultados de la Tabla \ref{tab:experimento_precision_dispositivo_max_fps_carga_alta_constante}, se pueden extraer varias conclusiones. Exportar el modelo con TensorRT manteniendo la precisión FP32 produce una aceleración de $\frac{183.29}{84.44} = 2.17\times$ en comparación con la ejecución del modelo sin optimizar. Esto demuestra que optimizar el modelo con TensorRT mejora significativamente el rendimiento, incluso sin reducir la precisión numérica. TensorRT aplica optimizaciones como la fusión de capas y la eliminación de operaciones redundantes, lo que resulta en una ejecución más eficiente. 


Para la precisión del modelo en GPU, los resultados muestran un rendimiento similar entre las ejecuciones con precisiones FP32, FP16 e INT8. Sin embargo, es importante destacar que las precisiones FP16 e INT8 logran un menor consumo energético. Esto se debe a que estas precisiones reducidas requieren menos recursos computacionales, lo que se traduce en una mayor eficiencia energética. En particular, la precisión INT8 se presenta como la opción más eficiente en términos de consumo energético, alcanzando un rendimiento de 1.96 Frames/W.


\begin{figure}[H]
   \centering
   \includegraphics[width=0.7\textwidth]{images/analisis_de_la_solucion/precision_device/tiempos_inferencia_gpu.png}
   \caption{Tiempos de ejecución de la etapa de inferencia para las diferentes precisiones y GPU.}
   \label{fig:tiempos_inferencia_gpu}
\end{figure}


Un análisis más detallado de los tiempos de la etapa de inferencia, como se observa en la Figura \ref{fig:tiempos_inferencia_gpu}, revela una reducción significativa en el tiempo de inferencia al utilizar precisiones más bajas en combinación con TensorRT.

Observando el tiempo en la CPU de la Tabla \ref{tab:experimento_precision_dispositivo_max_fps_carga_alta_constante}, se puede apreciar que la ejecución del modelo en CPU es completamente inviable para tiempo real, con una tasa de 0.93 FPS.

Al comparar el rendimiento de la GPU y la DLA, se observa que la DLA ofrece un rendimiento similar al de la GPU. Repasando la sección \ref{sec:jetson}, se puede ver que la DLA está diseñada para manejar cargas de trabajo específicas de inferencia de manera más eficiente.

Sin embargo, es crucial señalar que el modelo, tanto en FP16 como en INT8, no se ejecuta completamente en la DLA, ya que algunas operaciones no son compatibles con este dispositivo. Esto implica que la GPU también participa en el procesamiento.

Este fenómeno se hace evidente al observar que, en la precisión INT8, ninguna capa del modelo se ejecuta en la DLA, lo que significa que la GPU es la que realiza todo el procesamiento.

En la precisión FP16, como se observa en la Figura \ref{fig:exportacion_FP16_DLA}, la DLA se encarga de 6 de las 34 operaciones/capas del modelo, mientras que la GPU se encarga de las restantes 28.

En esencia, la DLA no está descargando completamente a la GPU, lo que limita su capacidad para operar de forma independiente y optimizar el rendimiento general del sistema.

En otros modelos más simples, la DLA podría encargarse de la mayoría de las operaciones, pero en el caso del modelo YOLO11n (tampoco fue posible para YOLOv5 y YOLOv8), la complejidad del modelo y las operaciones involucradas impiden que la DLA asuma una carga de trabajo significativa.


\begin{figure}[H]
   \centering
   \includegraphics[width=0.7\textwidth]{images/analisis_de_la_solucion/precision_device/exportación_FP16_DLA.png}
...
\begin{table}[H]
   ...
   En la Tabla \ref{tab:experimento_precision_dispositivo_30_fps}, se obsrvan las ejecuciones en tiempo real de los modelos con diferentes precisiones y dispositivos. Observando los resultados, como se ha mencionado anteriormente, la ejecución del modelo en CPU es completamente inviable para tiempo real, con una tasa de 0.79 FPS y 28.49 LFPS. En cuanto a la GPU, todas las precisiones (FP32, FP16 e INT8 con TensorRT) logran operar en tiempo real, con tasas de FPS de 28.42, 28.73 y 28.44 respectivamente, lo que indica que el sistema puede procesar los fotogramas a la velocidad requerida sin perder ninguno. La combinación de la GPU con la precisión INT8 es la más eficiente en términos de consumo energético, alcanzando 1.96 Frames/W.
   ...
   En esta sección se analizará el rendimiento de la solución propuesta variando el modo de energía del dispositivo (10W, 15W, 30W, MAXN) y el número de cores de la CPU habilitados (2, 4, 6 y 8 cores). Es importante mencionar que los perfiles de energía que se han utilizado son los que se encuentran disponibles en el dispositivo Jetson AGX Xavier por defecto, estos perfiles de energía también modifican la frecuencia máxima del procesador.
