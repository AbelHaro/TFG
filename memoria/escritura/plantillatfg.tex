...
\begin{figure}[H]
   \centering
   \includegraphics[width=0.8\textwidth]{images/analisis_de_la_solucion/variación_de_objetos_en_el_vídeo.png}
   \caption{Número de objetos en el vídeo de prueba.}
   \label{fig:variacion_objetos}
\end{figure}

La Figura \ref{fig:variacion_objetos} ilustra la fluctuación en el conteo de objetos a lo largo del vídeo de prueba. Esta variación es un factor clave en la evaluación del rendimiento del sistema, ya que una mayor densidad de objetos puede incrementar la carga computacional y, por ende, influir en el tiempo de procesamiento.

Para comprender a fondo el impacto de esta variación, es esencial analizar cómo afecta a cada etapa dentro de la arquitectura segmentada del sistema (descrita en la sección \ref{sec:segmentacion_etapas}).

La etapa de captura permanece inalterada por el número de objetos debido a sus dimensiones de fotograma fijas y a su tasa de captura constante. De manera similar, la etapa de inferencia, procesada por la GPU o DLA, mantiene un rendimiento estable, ya que su tarea principal es el procesamiento de imágenes, independientemente del conteo de objetos. Por el contrario, la etapa de seguimiento, que está limitada por la CPU y directamente influenciada por el número de objetos, experimenta variaciones en el tiempo de procesamiento proporcionales al conteo de objetos. Un mayor número de objetos aumenta la complejidad del algoritmo de seguimiento, lo que podría crear un cuello de botella. Por último, la etapa de escritura, también basada en la CPU, podría verse afectada por el volumen de datos generados en las etapas anteriores.


\begin{figure}[H]
   \centering
   \includegraphics[width=0.8\textwidth]{images/analisis_de_la_solucion/tiempos_etapa.png}
\caption{Ejecución temporal de las etapas del sistema durante el vídeo de prueba. (Segmentación por procesos con memoria compartida, YOLO11n (TensorRT FP16, GPU, MAXN)).}
   \label{fig:tiempos_etapa}
\end{figure}


La Figura \ref{fig:tiempos_etapa} presenta el tiempo de ejecución de cada etapa del sistema a lo largo del vídeo de prueba. Se observa que la etapa de captura se mantiene constante, mientras que la etapa de inferencia puede mostrar un ligero aumento en el tiempo de procesamiento a medida que aumenta el número de objetos. La etapa de seguimiento presenta un aumento significativo en su tiempo de ejecución. Esto indica que el algoritmo de seguimiento se ve afectado por la cantidad de objetos presentes, lo que puede limitar el rendimiento del sistema en situaciones con alta densidad de objetos. También se observa que las gráfica \ref{fig:variacion_objetos} y \ref{fig:tiempos_etapa} son inversamente proporcionales, lo que indica que a mayor número de objetos, menor es el tiempo de ejecución de la etapa de inferencia. Esto se debe a que el algoritmo de inferencia se optimiza para procesar múltiples objetos en paralelo, lo que reduce su tiempo de ejecución total.

En resumen, el rendimiento global del sistema emerge como una sinergia compleja entre estas etapas interconectadas, donde la eficiencia de cada una modula el comportamiento del conjunto.
