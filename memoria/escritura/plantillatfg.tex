...
\title{Detección de defectos en objects en movimiento mediante Redes Neuronales Convolucionales con optimizaciones específicas para hardware NVIDIA}
...
Este trabajo de fin de grado aborda el desafío de la detección automática de defectos en objetos que se desplazan en entornos industriales, como líneas de producción. Se presenta el diseño y la implementación de un sistema completo de visión artificial basado en tècniques de aprendizaje profundo.
...
Como se observa en la Figura~\ref{fig:consumo_electrico_datacenters}, el consumo eléctrico de los centros de datos en el mundo ha ido aumentando de forma exponencial, lo que plantea un desafío significativo para la sostenibilidad del crecimiento tecnológico \cite{challe6010117}. En el peor escenario, esta tendencia podría llevar a un incremento insostenible en la huella de carbono del sector tecnológico, mientras que en el mejor de los casos, la adopción de tecnologías eficientes podría moderar el crecimiento. Este aumento del consumo energético no solo afecta a los centros de datos, sino también a los dispositivos embebids i mòbils, on l'eficiència energètica és crucial per allargar la vida útil de les bateries i reduir l'impacte ambiental.

Per afrontar aquests reptes, s'han desenvolupat diverses tècniques d'optimització i compressió que redueixen la mida i la complexitat dels models neuronals mantenint el seu rendiment. Paral·lelament, han sorgit arquitectures hardware dissenyades específicament per a la inferència de models d'aprenentatge profund en entorns amb restriccions energètiques. En aquest context, els dispositius de la sèrie Jetson de NVIDIA destaquen per oferir un equilibri entre alt rendiment en tasques d'intel·ligència artificial i un consum energètic contingut, ideal per a aplicacions embebides de visió artificial.

La combinació de xarxes neuronals convolucionals i acceleradors hardware ha permès la creació de sistemes de visió artificial que poden detectar i classificar objectes en moviment, cosa que és essencial en aplicacions com la vigilància, la conducció autònoma i la robòtica.
...
\begin{align}
33.3 \times 10^{-3} &= 2.94 \times 10^{-4} \times n_{objetos} + 3.4 \times 10^{-3}\\
33.3 \times 10^{-3} - 3.4 \times 10^{-3} &= 2.94 \times 10^{-4} \times n_{objetos}\\
29.9 \times 10^{-3} &= 2.94 \times 10^{-4} \times n_{objetos}\\
n_{objetos} &= \frac{29.9 \times 10^{-3}}{2.94 \times 10^{-4}}\\
n_{objetos} &\approx 101.7
\end{align}
...
\begin{table}[H]
   ...
   \pgfplotstabletypeset[
       col sep=comma,
       header=true,
       columns={cantidad_objetos,frames_procesados,tiempo_ejecucion_s,FPS,LFPS,potencia_media_consumida_W,energia_consumida_J,frames_por_W,speedup,avg_GPU,avg_CPU},
       display columns/0/.style={column name=Cant.\ Objetos, column type={c}, string type},
