...
\begin{table}[H]
   ...
   \pgfplotstabletypeset[
       ...
       display columns/9/.style={column name=GPU Avg.*, column type={c}},
   ...
   \caption{Resultados del experimento con distintos tipos de segmentación a máxima capacidad.\protect\newline\footnotesize{
      *En la configuración de segmentación por hardware, se utilizan la GPU y los 2 núcleos DLA; sin embargo, solo se muestra la utilización de la GPU, ya que la herramienta de monitorización de NVIDIA no reporta la utilización media de la DLA.}}
...
La segmentación por hilos es la que ofrece el peor rendimiento. Esto se debe fundamentalmente al Global Interpreter Lock (GIL) de Python, que impide el paralelismo real entre hilos dentro de un mismo proceso, limitando la capacidad de aprovechar múltiples núcleos de CPU.
...
Como se puede observar en la Tabla \ref{tab:experimento_segmentacion_30_fps}, el tipo de segmentación impacta significativamente el rendimiento del sistema. La segmentación por hilos resulta inviable para tiempo real, con una tasa de fotogramas perdidos (LFPS) de 11.51 fps, lo que confirma los resultados de la ejecución a máxima capacidad. Aunque la segmentación por procesos mejora este valor a 7.29 LFPS, tampoco alcanza los requisitos de tiempo real. Por su parte, la segmentación basada en hardware logra operar en tiempo real, pero a costa de un mayor consumo energético. Finalmente, la segmentación por procesos con memoria compartida se destaca como la opción más eficiente, logrando el mejor rendimiento en tiempo real (LFPS de 0.91) con el menor consumo energético.
