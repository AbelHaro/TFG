...
\subsection{Detectores de 2 etapas}
Las arquitecturas de detección de objetos se pueden clasificar en dos categorías principales: \textit{\textbf{detectores de dos etapas}} y \textit{\textbf{detectores de una etapa}}. Los detectores de dos etapas, como R-CNN, Fast R-CNN y Faster R-CNN, funcionan en dos fases: primero generan propuestas de regiones (region proposals) y luego clasifican estas regiones. Este enfoque permite una mayor precisión, pero a costa de un mayor tiempo de procesamiento.

La primera arquitectura exitosa de detección de objetos basada en deep learning fue R-CNN (Regions with CNN features) \cite{girshick2014richfeaturehierarchiesaccurate}. Este modelo introdujo un enfoque de dos etapas que revolucionó el campo. En su primera fase, R-CNN utiliza un algoritmo de búsqueda selectiva (Selective Search) para generar aproximadamente 2,000 propuestas de regiones que podrían contener objetos. Este algoritmo de búsqueda selectiva divide la imágen en nodos y aristas, e iterativamente agrupa estas regiones en función del color, textura, tamaño y forma hasta que se obtienen las propuestas finales. En la figura \ref{fig:selective_search}\cite{explainningAI} se muestra un ejemplo de cómo funciona el algoritmo de búsqueda selectiva.
...
\subsection{Detectores de 1 etapa}
Los detectores de una etapa, como YOLO (You Only Look Once) y SSD (Single Shot MultiBox Detector), abordan la detección de objetos de manera diferente. En lugar de generar propuestas de regiones y clasificarlas por separado, estos modelos realizan la detección en una sola pasada a través de la red. Esto se traduce en un aumento significativo en la velocidad de procesamiento, lo que los hace ideales para aplicaciones en tiempo real.

\subsection{Evolución de las arquitecturas de detección}
\begin{enumerate}
