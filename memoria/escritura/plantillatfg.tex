...
Las \gls{cnn} o redes neuronales convolucionales son un tipo específico de red neuronal profunda. Estas redes están diseñadas para procesar imágenes y extraer características relevantes de manera eficiente, lo que las hace especialmente adecuadas para tareas de visión por computador.
...
Frente a las limitaciones de las CPUs, han surgido arquitecturas de dominio específico diseñadas para optimizar el rendimiento en tareas concretas. Entre ellas destacan las \gls{fpga} (Field Programmable Gate Arrays), los \gls{asic} (Application-Specific Integrated Circuits) y las \gls{gpu} (Graphics Processing Units). A diferencia de las CPUs de propósito general, estas arquitecturas sacrifican versatilidad para lograr una eficiencia superior en operaciones específicas como el procesamiento gráfico, la inteligencia artificial o el procesamiento de señales.

Las \gls{fpga} son circuitos reconfigurables que pueden programarse después de su fabricación, ofreciendo flexibilidad para adaptarse a distintas aplicaciones, aunque su programación requiere conocimientos especializados. Los \gls{asic}, por otro lado, se diseñan a medida para una única tarea, alcanzando la máxima eficiencia y rendimiento, pero a costa de un diseño costoso, largo y la imposibilidad de reprogramación. Un ejemplo notable de \gls{asic} son las \gls{tpu} (Tensor Processing Units) desarrolladas por Google, optimizadas específicamente para acelerar las cargas de trabajo de aprendizaje automático neuronal.

Las \gls{gpu} son un ejemplo paradigmático de arquitectura de dominio específico orientada al paralelismo masivo. Su diseño \textit{many-core} integra miles de núcleos de procesamiento más simples en un único chip. Esta estructura les permite ejecutar una gran cantidad de hilos de forma concurrente, lo que las hace ideales para cargas de trabajo intensivas y fácilmente paralelizables, como las operaciones matriciales típicas del aprendizaje profundo. Dada su capacidad para acelerar significativamente las tareas de inteligencia artificial, las \gls{gpu} constituyen la arquitectura hardware central utilizada en este proyecto.

El modelo de programación de las \gls{gpu} está basado en la ejecución masiva de hilos, organizados en bloques y rejillas (\textit{blocks} y \textit{grids}), según la terminología de CUDA, el modelo de programación desarrollado por NVIDIA. Cada hilo ejecuta la misma función, conocida como \textit{kernel}, pero opera sobre diferentes fragmentos de datos. Este enfoque, conocido como SIMT (Single Instruction, Multiple Threads), permite aprovechar al máximo el paralelismo inherente a muchas aplicaciones de inteligencia artificial, como el entrenamiento y la inferencia de redes neuronales.
...
\end{figure}
La figura \ref{fig:cpu_vs_gpu}\cite{enccs_gpu_ecosystem} ilustra las diferencias arquitectónicas entre CPU y GPU. Las CPUs constan de un número reducido de núcleos (generalmente entre 4 y 64) optimizados para un alto rendimiento secuencial, donde cada núcleo dispone típicamente de caché y unidad de control privadas. En cambio, las \gls{gpu} se basan en una arquitectura \textit{many-core}, integrando miles de núcleos más simples diseñados para el paralelismo masivo, aunque con menor rendimiento individual por núcleo. Estos núcleos de \gls{gpu} suelen compartir recursos como la caché y las unidades de control, lo que posibilita empaquetar una mayor cantidad de unidades de procesamiento en el mismo chip y, consecuentemente, alcanzar una mayor densidad de cómputo.


NVIDIA ha sido una figura central en la evolución de la aceleración de IA, realizando contribuciones clave que han modelado el campo. Fue pionera en la computación de propósito general en \gls{gpu} (GPGPU) con la introducción de CUDA en 2006. Esta plataforma permitió utilizar la masiva capacidad de procesamiento paralelo de las \gls{gpu} para tareas computacionales generales, extendiendo su uso más allá de los gráficos tradicionales. La programación de \gls{gpu} se realiza utilizando lenguajes y APIs especializadas como CUDA y OpenCL, que otorgan al desarrollador un control explícito sobre la distribución de datos y tareas entre los miles de núcleos disponibles.
...
\section{Entrenamiento del modelo} \label{sec:entrenamiento_modelo}
...
Como, se ha mencionado en la sección \ref{sec:one_stage_detectors}, el modelo YOLO11 presenta varias variantes, cada una con diferentes tamaños y complejidades. Estas diferencias se observan en la Tabla \ref{tab:yolo11_comparison}, donde se comparan las variantes de YOLO11 en términos de latencia, precisión y número de parámetros. Las métricas presentan una clara correlación: a medida que aumenta el tamaño del modelo, también lo hacen la latencia y el número de parámetros, mientras que la precisión (mAP) tiende a mejorar. Esta relación es fundamental para la selección del modelo adecuado, ya que permite equilibrar la necesidad de velocidad de procesamiento con la precisión requerida para la tarea específica de detección y seguimiento de objetos.

Para este trabajo, se optó por la prueba experimental de las variantes YOLO11n, YOLO11s, YOLO11m y YOLO11l, por ello se han entrenado y evaluado estas variantes.
...
Para el entrenamiento del modelo se han seleccionado los siguientes hiperparámetros:
\begin{itemize}
...
\begin{table}[H]
   ...
   \begin{tabular}{l | c | c | c | c | c}
      ...
      \bottomrule
   \end{tabular}
   \caption{Comparativa del rendimiento de modelos YOLO11 con métricas de entrenamiento y evaluación.}
   \label{tab:yolo11_metrics}
\end{table}

La Tabla \ref{tab:yolo11_metrics} resume los resultados del entrenamiento. Se observa una tendencia general: a medida que aumenta la complejidad del modelo (desde YOLO11n hasta YOLO11l), tanto la precisión como el recall experimentan una mejora. Esto sugiere que los modelos de mayor tamaño poseen una capacidad superior para aprender representaciones de características más ricas y discriminativas, resultando en una detección de objetos más efectiva. No obstante, esta mejora en el rendimiento se acompaña de un incremento en el tiempo de entrenamiento, evidenciando el inherente compromiso entre la capacidad del modelo y la eficiencia computacional del proceso de aprendizaje.

\begin{figure}[H]
   \centering
   \includegraphics[width=0.8\textwidth]{excels/entrenamiento/loss_plot.png}
   \caption{Curvas de pérdida de entrenamiento y validación para las diferentes tallas de YOLO11.}
   \label{fig:loss_curves}
\end{figure}

La Figura \ref{fig:loss_curves} presenta las curvas de pérdida (loss) durante el entrenamiento y la validación para las distintas variantes de YOLO11. Estas gráficas ilustran la progresiva disminución de la función de pérdida a lo largo de las épocas, un indicador clave de que el modelo está aprendiendo eficazmente a generalizar a partir de los datos de entrenamiento para la tarea de detección de objetos. Es notable que la pérdida de validación también sigue una tendencia descendente, lo que sugiere una buena generalización y la ausencia de un sobreajuste significativo a los datos de entrenamiento. Se aprecia un descenso pronunciado de la pérdida en las épocas iniciales, indicativo de una rápida asimilación de patrones por parte del modelo. Conforme avanza el entrenamiento, la tasa de reducción de la pérdida disminuye gradualmente hasta alcanzar una meseta, un comportamiento característico en el entrenamiento de modelos de aprendizaje profundo que señala la convergencia del modelo hacia un mínimo local de la función de pérdida, donde las mejoras adicionales se vuelven marginales.
...
\begin{itemize}
   \item \textbf{Tasa de fotogramas por segundo (FPS):} Número de fotogramas procesados por segundo. Esta métrica es fundamental para evaluar la capacidad del sistema para operar en tiempo real.
   \item \textbf{Tasa de fotogramas perdidos (LFPS):} Número de fotogramas perdidos por segundo. Esta métrica indica la capacidad del sistema para mantener el ritmo con la entrada de fotogramas.
   \item \textbf{Potencia consumida media (W):} Potencia media consumida por el sistema durante la ejecución. Se calcula como el promedio de la potencia medida en vatios a lo largo del tiempo de ejecución. Esta métrica es crucial para evaluar la eficiencia energética del sistema.
   \item \textbf{Energía consumida (J):} Representa la energía total consumida por el sistema para procesar la secuencia completa. Se calcula integrando la potencia a lo largo del tiempo de ejecución. Esta métrica ofrece una visión global de la eficiencia energética.
   \item \textbf{Fotogramas por vatio (Frames/W):} Indica cuántos fotogramas se procesan por cada vatio de potencia consumido. Esta métrica es útil para evaluar la eficiencia del sistema en términos de rendimiento por unidad de energía. Se calcula de la siguiente manera:
   \begin{equation}
   ...
   \item \textbf{Speedup:} Compara la mejora en la velocidad de procesamiento. Se calcula como el cociente entre el tiempo de ejecución de la configuración de referencia (la más lenta) y el tiempo de la configuración evaluada. Un valor mayor a 1 indica una ejecución más rápida.
\end{itemize}
...
\begin{enumerate}
   \item \textbf{Carga baja y constante:} Un vídeo que mantiene una cantidad constante media de 43 objetos en cada fotograma. Este escenario permite evaluar el rendimiento base del sistema bajo una carga predecible y reducida.
   \item \textbf{Carga alta y constante:} Un vídeo que presenta una cantidad constante media de 84 objetos por fotograma. Este escenario somete al sistema a una carga significativamente mayor, permitiendo identificar posibles cuellos de botella bajo condiciones de alta densidad de objetos.
   \item \textbf{Carga variable:} Un vídeo donde la cantidad de objetos fluctúa dinámicamente a lo largo de su duración, variando entre 0 y un máximo de 180 objetos. Este escenario simula condiciones más realistas y complejas, donde el sistema debe adaptarse a cambios abruptos en la carga de trabajo.
\end{enumerate}
...
Para el resto de la configuración del sistema durante estas pruebas específicas sobre la cantidad de objetos, se ha empleado la segmentación por procesos con memoria compartida. El modelo de detección de objetos utilizado fue YOLO11n, optimizado mediante NVIDIA TensorRT para su ejecución en la GPU. Se configuró para operar con precisión FP16 y con el perfil de energía del dispositivo Jetson ajustado al modo de máxima potencia (MAXN). Esta configuración se seleccionó con el objetivo de maximizar el rendimiento del sistema y evaluar su capacidad de respuesta y robustez bajo condiciones exigentes impuestas por la variación en la densidad de objetos. Las pruebas presentadas en esta sección se realizaron siguiendo la metodología de ejecución a máxima capacidad descrita en la sección \ref{sec:metodologia}.

\begin{figure}[H]
   \centering
   \includegraphics[width=0.8\textwidth]{images/analisis_de_la_solucion/cantidad_objetos/cantidad_objetos_3_videos.png}
   \caption{Cantidad de objetos en los vídeos de prueba.}
   \label{fig:3_videos}
\end{figure}

La Figura \ref{fig:3_videos} ilustra la fluctuación en el conteo de objetos a lo largo de los tres vídeos de prueba utilizados: carga baja y constante, carga alta y constante, y carga variable. Esta variación en la densidad de objetos es un factor clave en la evaluación del rendimiento del sistema, ya que una mayor cantidad de objetos puede incrementar la carga computacional, especialmente en las etapas dependientes de la CPU.

\begin{figure}[H]
   \centering
   \includegraphics[width=0.8\textwidth]{excels/inferencia/cantidad_objetos/fps_por_frame.png}
   \caption{FPS por fotograma en función de la cantidad de objetos para los tres vídeos de prueba.}
   \label{fig:fps_por_frame}
\end{figure}

Observando la Figura \ref{fig:fps_por_frame}, se puede apreciar cómo el rendimiento global del sistema, medido en fotogramas por segundo (FPS), se ve afectado por la cantidad de objetos presentes en el flujo de vídeo. En el vídeo de carga baja y constante (línea azul), el sistema logra mantener un rendimiento estable, promediando alrededor de 45-50 FPS. Para el vídeo de carga alta y constante (línea roja), el rendimiento promedio disminuye a aproximadamente 30 FPS, indicando que la mayor densidad de objetos impone una carga adicional. En el caso del vídeo con carga variable (línea verde), el rendimiento fluctúa significativamente: alcanza más de 60 FPS cuando hay menos de 20 objetos, pero desciende a cerca de 20 FPS cuando la cantidad de objetos supera los 100. Esta variabilidad en el rendimiento sugiere que ciertas etapas del sistema son sensibles al número de objetos.
...
Por el contrario, la etapa de seguimiento (línea verde), que se ejecuta en la CPU, muestra una clara correlación entre su tiempo de ejecución y la cantidad de objetos. A medida que aumenta el número de objetos (como se ve en la curva de carga variable de la Figura \ref{fig:3_videos}), el tiempo que tarda la etapa de seguimiento también aumenta significativamente. Esto se debe a que el algoritmo BYTETrack debe gestionar más trayectorias, realizar más comparaciones para la asociación de datos y actualizar más filtros de Kalman. Este comportamiento indica que la etapa de seguimiento puede convertirse en un cuello de botella cuando la densidad de objetos es alta.

Finalmente, la etapa de escritura (línea roja), también dependiente de la CPU, muestra un ligero incremento en su tiempo de ejecución a medida que aumenta el número de objetos. Esto es esperable, ya que debe procesar y registrar la información de un mayor número de detecciones y trayectorias. Sin embargo, su impacto en el rendimiento general parece ser menor en comparación con la etapa de seguimiento bajo las condiciones probadas.
...
Repasando la sección \ref{sec:segmentacion_etapas}, se han implementado cinco tipos de segmentación:
\begin{enumerate}
   \item Segmentación secuencial (subsección \ref{sub:secuencial}): Cada etapa del sistema se ejecuta de forma consecutiva.
...
\section{Talla del modelo} \label{sub:talla_modelo}
