...
\addcontentsline{toc}{chapter}{Agradecimientos}  % Opcional si quieres que aparezca en el índice
...
\begin{table}[H]
   ...
   \begin{tabular}{llc|c|ccl} % Familia (l), Variante (l), Tamaño (c) | Parámetros (M) (c) | Lat CPU (c), Lat GPU (c), GPU Tipo (l)
      \toprule
      Familia & Variante & Tamaño (px) & Parámetros (M) & \begin{tabular}[c]{@{}c@{}}Latencia CPU\\ONNX (ms)\end{tabular} & \begin{tabular}[c]{@{}c@{}}Latencia\\GPU (ms)\end{tabular} & \begin{tabular}[c]{@{}c@{}}GPU\\(para Latencia)\end{tabular} \\
      \midrule
   ...
   \caption[Análisis comparativo de variantes de YOLO (v5, v8, 11) indicando tamaño, parámetros, latencias CPU/GPU y la GPU específica utilizada para la medición de latencia GPU]{Análisis comparativo de variantes de YOLO (v5, v8, 11) indicando tamaño, parámetros, latencias CPU/GPU y la GPU específica utilizada para la medición de latencia GPU. Datos extraídos de \cite{yolo11_ultralytics}.}
...
\begin{table}[H]
   ...
   \pgfplotstabletypeset[
      ...
      %  display columns/12/.style={column name=T. Postpro. (ms), column type={c}, fixed, precision=2},
       display columns/8/.style={column name={mAP$_{50-95}$ $\uparrow$}, column type={c}, fixed, precision=4},
       every head row/.style={before row=\toprule, after row=\midrule},
       every last row/.style={after row=\bottomrule},
       empty cells with={NaN}
...
\begin{table}[H]
   ...
   \pgfplotstabletypeset[
      ...
      display columns/1/.style={column name=\begin{tabular}[c]{@{}c@{}}Precisión\end{tabular}, column type={c}, string type},
...
\begin{table}[H]
   ...
   \pgfplotstabletypeset[
      ...
      \caption[Resultados del experimento con distintas precisiones y aceleradores a 30 fps con un vídeo de carga alta y constante]{Resultados del experimento con distintas precisiones y aceleradores a 30 fps con un vídeo de carga alta y constante.}
