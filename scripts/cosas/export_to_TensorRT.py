...
def export_model(model_name, device, batch_size=1):
    print(
        f"[EXPORT TO TensorRT] Exporting model {model_name} to TensorRT with hardware {device} and batch size {batch_size}"
    )
    ...
    target_path = os.path.join(
        MODEL_BASE_PATH,
        f"{model_name.replace('.pt', '')}_{precision_suffix}_{hardware_suffix}.engine",
    )
